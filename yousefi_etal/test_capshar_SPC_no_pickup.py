...
for i in range(kk):
    ...
    if os.path.isfile('./results/mdl_HAR.hdf5'):
        os.remove('./results/mdl_HAR.hdf5')

    # train the model
    adam = keras.optimizers.Adam(learning_rate=lr)
    model.compile(optimizer=adam, loss=[marginLoss, 'mse'], loss_weights=[1., 0.392], metrics=['accuracy'])

    #callbacks
    # saver = CustomSaver() # ensemble learning saver
    # checkpoint = ModelCheckpoint("./results/mdl_HAR.hdf5", monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')
