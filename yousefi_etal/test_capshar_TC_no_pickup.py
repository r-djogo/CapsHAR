...
x_fall, y_fall = shuffle(x_fall, y_fall, random_state=1)
x_run, y_run = shuffle(x_run, y_run, random_state=1)
x_sitdown, y_sitdown = shuffle(x_sitdown, y_sitdown, random_state=1)
x_standup, y_standup = shuffle(x_standup, y_standup, random_state=1)
x_walk, y_walk = shuffle(x_walk, y_walk, random_state=1)
...
for i in range(kk):
    ...
    x_gen = tf.keras.layers.Dense(units=20*30*32, activation=tf.nn.relu)(input_gen)
    x_gen = tf.keras.layers.Reshape(target_shape=(20, 30, 32))(x_gen)
    x_gen = tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=16, strides=(5,3), padding='same', activation='relu')(x_gen)
    x_gen = tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=16, strides=(5,1), padding='same')(x_gen)
    ...
    if os.path.isfile('./results_6/mdl_HAR.hdf5'):
        os.remove('./results_6/mdl_HAR.hdf5')
    ...
    #callbacks
    # saver = CustomSaver() # ensemble learning saver
    # checkpoint = ModelCheckpoint("./results_6/mdl_HAR.hdf5", monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')
    log = tf.keras.callbacks.CSVLogger('./results_6/log.csv', append=True)
    ...
    model.save('./results_6/final_model.hdf5')
...
f_res = open("./results_6/matrices.txt", "w")
...
if n_classes == 6:
    plot_conf_matrix_6_file(avg_matrix, "./results_6/conf_matrix.png")
elif n_classes == 7:
    plot_conf_matrix_7(avg_matrix)
else:
    print(avg_matrix)
