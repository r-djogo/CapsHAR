...
for a_idx, alpha in enumerate(alphas):
    ...
    # make directory
    if not os.path.exists(f"./alpha_results_no_pickup/{str(alpha)}"):
        os.makedirs(f"./alpha_results_no_pickup/{str(alpha)}")
    ...
    for i in range(kk):
        ...
        if os.path.isfile(f"./alpha_results_no_pickup/{str(alpha)}/mdl_HAR.hdf5"):
            os.remove(f"./alpha_results_no_pickup/{str(alpha)}/mdl_HAR.hdf5")
        ...
        #callbacks
        log = tf.keras.callbacks.CSVLogger(f"./alpha_results_no_pickup/{str(alpha)}/log.csv", append=True)
        ...
        model.save(f"./alpha_results_no_pickup/{str(alpha)}/final_model.hdf5")
    ...
    f_res = open(f"./alpha_results_no_pickup/{str(alpha)}/matrices.txt", "w")
    ...
    if n_classes == 6:
        plot_conf_matrix_6_file(avg_matrix, f"./alpha_results_no_pickup/{str(alpha)}/conf_matrix.png")
    elif n_classes == 7:
        plot_conf_matrix_7_file(avg_matrix, f"./alpha_results_no_pickup/{str(alpha)}/conf_matrix.png")
    else:
        print(avg_matrix)
