...
def csv_import():
    ...
    for i in ["bed", "fall", "pickup", "run", "sitdown", "standup", "walk"]:
        ...
        num_lines = sum(1 for l in open("/home/..." + str(i) + ".csv"))
        skip_idx = [x for x in range(1, num_lines) if x % SKIPROW !=0]

        xx = np.array(pd.read_csv("/home/..." + str(i) + ".csv", header=None, skiprows = skip_idx))
        yy = np.array(pd.read_csv("/home/..." + str(i) + ".csv", header=None, skiprows = skip_idx))
