...
from utils.layers import FCCaps, Length, PrimaryCaps
...
for i in range(kk):
    ...
    t_start = time.time()
    history = model.fit(wifi_x_train, wifi_y_train, batch_size=batch_size, epochs=epochs,
                        validation_data = (wifi_x_validation, wifi_y_validation),
                        callbacks=[log, lr_decay_cb], shuffle=True, verbose=verbose)
    ...
    pred, inference_time = load_model_predictions(wifi_x_validation, batch_size, name='./results/final_model.hdf5')
    ...
    del wifi_x_train, wifi_y_train, wifi_x_validation, wifi_y_validation, \
        input, x, activity_caps, output, model, \
        adam, log, lr_decay_cb
    gc.collect()
...
for hist in hists:
    f_res.write("acc:")
    f_res.write(str(hist.history['accuracy']))
    f_res.write("\nval_acc:")
    f_res.write(str(hist.history['val_accuracy']))
    f_res.write("\nloss:")
    f_res.write(str(hist.history['loss']))
    f_res.write("\nval_loss:")
    f_res.write(str(hist.history['val_loss']))
    f_res.write("\n")
f_res.close()

if n_classes == 6:
    plot_conf_matrix_6(avg_matrix)
elif n_classes == 7:
