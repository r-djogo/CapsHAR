...
def plot_conf_matrix_10(matrix, title=''):
    fig=plt.figure(figsize=(6,5))
    ax = plt.subplot()
    sns.heatmap(matrix, annot=True, fmt='g', ax=ax); 
    acc = np.round(np.mean(matrix.diagonal()), 1)
    ...
    ax.xaxis.set_ticklabels(['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'])
    ax.yaxis.set_ticklabels(['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'])
    plt.savefig('./results/mdl_conf_matrix.png', bbox_inches='tight')
    plt.close()

def plot_conf_matrix_10_file(matrix, file, title=''):
    fig=plt.figure(figsize=(6,5))
    ax = plt.subplot()
    sns.heatmap(matrix, annot=True, fmt='g', ax=ax); 
    acc = np.round(np.mean(matrix.diagonal()), 1)
