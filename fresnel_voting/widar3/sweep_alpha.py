...
def load_data(path_to_data, motion_sel):
    ...
    for data_root, data_dirs, data_files in os.walk(path_to_data):
        ...
        for i, data_file_name in enumerate(data_files):
            ...
            label.append([label_1, location, orientation])
...
def assemble_caps(input_shape, n_class):
    ...
    x = tf.keras.layers.BatchNormalization()(x)
    x = PrimaryCaps(F=16, K=8, N=300, D=8, s=2)(x)
    ...
    x_gen = tf.keras.layers.Dense(units=58*9*1, activation="relu")(input_gen)
    x_gen = tf.keras.layers.Reshape(target_shape=(58, 9, 1))(x_gen)
    R = 10
    x_gen = tf.keras.layers.Conv2D(filters=R**2, kernel_size=5, padding='same')(x_gen)
    x_gen = tf.nn.depth_to_space(x_gen, R) # 580x90x1 output
    x_gen = tf.keras.layers.Cropping2D(cropping=((2, 3), (0, 0)))(x_gen) # 575x90x1 output
...
print('Loaded dataset of ' + str(label.shape[0]) + ' samples, each sized ' + str(data[0,:,:].shape))
...
for a_idx, alpha in enumerate(alphas):
    ...
    for run in range(n_runs):
        with tf.compat.v1.Session() as sess:
            ...
            [data_train_all, data_test_all, label_train, label_test] = train_test_split(data, label, test_size=fraction_for_test)
            # print('\nTrain on ' + str(label_train.shape[0]) + ' samples\n' +\
            #     'Test on ' + str(label_test.shape[0]) + ' samples\n')
            data_train_all = np.expand_dims(data_train_all, axis=3)
            data_test_all = np.expand_dims(data_test_all, axis=3)
            # print(data_train_all.shape, data_test_all.shape)
            len_train = label_train.shape[0] * 0.9
            len_test = label_test.shape[0]

            # One-hot encoding for train data
            label_train_class = onehot_encoding(label_train[:,0], N_MOTION)
            label_test_class = onehot_encoding(label_test[:,0], N_MOTION)
            ...
            for ap_id, ap_name in enumerate(APs):
                ...
                data_train = data_train_all[:,:,ap_id*90:(ap_id+1)*90,:]
                data_test = data_test_all[:,:,ap_id*90:(ap_id+1)*90,:]

                # # model = assemble_model(input_shape=(T_MAX, 20, 20, 1), n_class=N_MOTION)
                model, model_test = assemble_caps(input_shape=(data_train.shape[1], data_train.shape[2], data_train.shape[3]), n_class=N_MOTION)

                if run == 0 and a_idx == 0 and ap_id == 0:
                    model_test.summary()
                ...
                #Train
                t_start = time.time()
                history = model.fit([data_train, label_train_class], [label_train_class, data_train], batch_size=n_batch_size, epochs=n_epochs,
                ...
                del data_test, adam, log, lr_decay_cb, callbacks, model, model_test, pred
                gc.collect()

            # save true labels
            predictions['label'+str(run)] = label_test

            # voting
            voted_pred = np.zeros((len_test, N_MOTION))
            for ap_id, ap_name in enumerate(APs):
                voted_pred += weights[ap_id] * predictions[str(run)+ap_name]

            # get performance metrics
            matrices.append(create_conf_matrix(label_test_class, voted_pred))
            MCC.append(matthews_corrcoef(np.argmax(label_test_class, axis=1), np.argmax(voted_pred, axis=1)))
            F1.append(f1_score(np.argmax(label_test_class, axis=1), np.argmax(voted_pred, axis=1), average='micro'))
            B_ACC.append(balanced_accuracy_score(np.argmax(label_test_class, axis=1), np.argmax(voted_pred, axis=1)))
            ACC.append(accuracy_score(np.argmax(label_test_class, axis=1), np.argmax(voted_pred, axis=1)))
            
            print([round(b,3) for b in B_ACC])

            del label_train, label_train_class, label_test, label_test_class, data_train_all, data_test_all
            gc.collect()
