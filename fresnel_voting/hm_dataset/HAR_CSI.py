...
def combine_ap_data(data, labels, orients, n_APs):
    """
    ...
    for i in range(1, max(labels)+1, n_APs):
        ...
        y_orients.extend([orients[ori_idx]] * len(AP_data[0]))
...
def plot_conf_matrix_4_file(matrix, file, title=''):
    fig=plt.figure(figsize=(6,5))
    ax = plt.subplot()
    sns.heatmap(matrix, annot=True, fmt='g', ax=ax);
    acc = np.round(np.mean(matrix.diagonal()), 1)
...
def plot_conf_matrix_5_file(matrix, file, title=''):
    fig=plt.figure(figsize=(6,5))
    ax = plt.subplot()
    sns.heatmap(matrix, annot=True, fmt='g', ax=ax); 
    acc = np.round(np.mean(matrix.diagonal()), 1)
