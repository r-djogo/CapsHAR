...
routings = 3 # number of routing iterations on CapsNet
...
for run in range(runs):
    ...
    x = tf.keras.layers.BatchNormalization()(x)
    x = PrimaryCaps(F=16, K=8, N=2808, D=8, s=2)(x)
    ...
    x_gen = tf.keras.layers.Dense(units=20*24*32, activation=tf.nn.relu)(input_gen)
    x_gen = tf.keras.layers.Reshape(target_shape=(20, 24, 32))(x_gen)
    x_gen = tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=16, strides=(5,8), padding='same', activation='relu')(x_gen)
    x_gen = tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=16, strides=(5,5), padding='same')(x_gen)
...
if len(data_groupings) == 5:
    plot_conf_matrix_5(avg_matrix)
elif len(data_groupings) == 4:
    plot_conf_matrix_4(avg_matrix)
elif len(data_groupings) == 17:
    plot_conf_matrix_17(avg_matrix)
else:
    print(avg_matrix)